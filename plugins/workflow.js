...
import PixlRequest from '../pixl/request.mjs';
	...
	async function poll() {
		...
		var table = {
			...
			rows: Object.keys(jobStatus).map(key=> [
				jobStatus[key].seq,
				`<span style="${jobStatus[key].code % 255 ? 'color:red' : ''}"><b>${encode(jobStatus[key].title) || '[Unknown]'}</b></span>` + (jobStatus[key].arg ? ': ' + encode(jobStatus[key].arg) : ''),
				...
				`${encode(jobStatus[key].description)}`.substring(0,120)
