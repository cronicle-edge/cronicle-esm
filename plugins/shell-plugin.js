...
import { writeFileSync, appendFileSync, unlink } from 'fs';
import { tmpdir } from 'os';
import { spawn } from 'child_process';
import { join } from 'path';
import JSONStream from '../pixl/json-stream.mjs';
import { getDateArgs, getErrorDescription } from '../pixl/tools.mjs';
...
stream.on('json', function (job) {
	...
	let script_file = join(tmpdir(), 'cronicle-script-temp-' + job.id + '.sh');
	writeFileSync(script_file, job.params.script, { mode: "775" });
	...
	const child = spawn(child_exec, child_args, {stdio: ['pipe', 'pipe', 'pipe']});
	...
	child.on('error', function (err) {
		...
		unlink(script_file, function (err) { ; });
	});
	...
	child.stderr.on('data', function (data) {
		...
		appendFileSync(job.log_file, data);
	});
