...
import { writeFile, appendFileSync, chmodSync, writeFileSync, openSync, closeSync, existsSync, readFileSync, statSync, unlink, stat, createReadStream, open, fstat, read, close } from 'fs';
import { tmpdir } from 'os';
import { resolve, join, basename } from 'path';
import { parse as sqparse } from 'shell-quote';
import { createGzip } from 'zlib';
import { createHmac } from 'crypto';

import { timeNow, hashValuesToArray, findObjectsIdx, pluralize, copyHash, mergeHashes, sub, randArray, findObject, getTextFromSeconds, getpwnam, getDateArgs, getErrorDescription, normalizeTime, composeQueryString, digestHex, getTextFromBytes, shortFloat, isaHash, mergeHashInto, numKeys } from "../pixl/tools.mjs";
import JSONStream from "../pixl/json-stream.mjs";
import PixlMail from '../pixl/mail.mjs';
import Component from '../pixl/component.mjs';

export default class Job extends Component {

	launchOrQueueJob(event, callback) {
		...
		this.launchJob(event, function (err, jobs) {
			if (err && event.queue) {
				...
				if (!queue_max || (self.eventQueue[event.id] < queue_max)) {
					...
					if (!event.now) event.now = timeNow(true);
	...
	launchJob(event, callback) {
		...
		series([
			...
			function (err) {
				...
				var job_list = hashValuesToArray(all_jobs);

				// check running jobs vs. max children
				if (orig_event.max_children) {
					var event_jobs = findObjectsIdx(job_list, { 'event': event.id });
					if (event_jobs.length >= orig_event.max_children) {
						// too many event children running
						return callback(new Error("Maximum of " + orig_event.max_children + " " + pluralize("job", orig_event.max_children) + " already running for event: " + event.title));
					}
				}

				if (category.max_children) {
					var cat_jobs = findObjectsIdx(job_list, { 'category': event.category });
					if (cat_jobs.length >= category.max_children) {
						// too many category children running
						return callback(new Error("Maximum of " + category.max_children + " " + pluralize("job", category.max_children) + " already running for category: " + category.title));
					}
				}
				
				var max_jobs = self.server.config.get('max_jobs') || 0;
				if (max_jobs && (job_list.length >= max_jobs)) {
					return callback( new Error("Global maximum of " + max_jobs + " " + pluralize("job", max_jobs) + " already running.") );
				}
				...
				for (var idx = 0, len = servers.length; idx < len; idx++) {
					...
					job.time_start = timeNow();
					job.hostname = worker.hostname;
					job.event = event.id;
					job.params = event.params || {};
					job.now = event.now || timeNow(true);
					...
					if (job.event in jobMemos) job.last_memo = jobMemos[job.event]
					...
					job.env = mergeHashes(plugin.env || {}, self.server.config.get('job_env') || {});
					if (plugin.secret) job.env['PLUGIN_SECRET'] = plugin.secret;
					job.env['BASE_URL'] = 'http://' + self.server.hostname + ':' + self.server.config.get('WebServer').http_port
					...
					if (job.params.script && job.params.sub_params) job.params.script = sub(job.params.script, xparams);
					...
					if (event.multiplex && event.stagger && (idx > 0)) {
						...
						job.when = timeNow() + (event.stagger * idx);
					...
					// fire web hook
					var hook_data = mergeHashes(job, { action: 'job_start' });
					...
					if (hook_text_templates[hook_data.action]) {
						hook_data.text = sub(hook_text_templates[hook_data.action], hook_data);
						...
						if (job.web_hook_start) {
							let wh_data = mergeHashes(hook_data, {}) // copy hook_data
	...
	chooseServer(candidates, event) {
		...
		switch (event.algo || 'random') {
			...
			case "least_cpu":
				...
				server = findObject(candidates, { hostname: least_hostname });
				break;
	...
	launchLocalJob(job) {
		...
		if (job.when && (job.when > timeNow())) {
			this.logDebug(6, "Job " + job.id + " will be delayed for " +
				getTextFromSeconds(job.when - timeNow()));
		...
		if (this.multi.manager || job.detached) {
			writeFile(job.log_file.replace(/\.log$/, '.json'), JSON.stringify(job), function (err) {
				if (err) self.logError('job', "Failed to write JSON job file: " + job.log_file.replace(/\.log$/, '.json') + ": " + err);
			});
		}

		let job_env = mergeHashes(process.env, job.env || {})

		if(job.secret) {
			try {
                job_env = mergeHashes(job_env, self.decryptObject(job.secret));
			}
			catch (e) {
		...
		if (job.is_wf || job.plugin == 'workflow') { 
			job_env['WF_SIGNATURE'] = 'sha1=' + createHmac("sha1", self.server.config.get('secret_key')).update(job.id).digest("hex")
		}
		...
		var user_info = getpwnam(child_opts.uid, true);
		if (user_info) {
		...
		if (job.params) {
			...
			if (self.server.config.get('inline_params')) {
				...
				for (var key in job.params) {
					child_opts.env[key.toUpperCase()] = job.params[key]
				}				
			}
		}
		...
		if (job.detached) {
			...
			var temp_file = join(tmpdir(), 'cronicle-job-temp-' + job.id + '.json');

			// tell child where the queue dir is
			job.queue_dir = resolve(this.server.config.get('queue_dir'));

			// write job file
			writeFileSync(temp_file, JSON.stringify(job));
			chmodSync(temp_file, "777");
			...
			try {
				child = spawn(resolve("bin/run-detached.js"), ["detached", temp_file], child_opts);
			}
			catch (err) {
				job.pid = 0;
				job.code = 1;
				job.description = "Child process error: " + getErrorDescription(err);
				this.logError("child", job.description);
				//this.activeJobs[job.id] = job;
				this.finishLocalJob(job);
				return;
			...
			// attach streams
			worker.log_fd = openSync(job.log_file, 'a');
			child_opts.stdio = ['pipe', 'pipe', worker.log_fd];

			// spawn child
			try {
				child = spawn(child_cmd, child_args, child_opts);
				...
				if (worker.log_fd) { closeSync(worker.log_fd); worker.log_fd = null; }
				job.pid = 0;
				job.code = 1;
				job.description = "Child spawn error: " + child_cmd + ": " + getErrorDescription(err);
				this.logError("child", job.description);
			...
			stream.on('text', function (line) {
				...
				appendFileSync(job.log_file, line);
			});
			...
			stream.on('error', function (err, text) {
				...
				if (text) appendFileSync(job.log_file, text + "\n");
			});
	...
	detachedJobUpdate(data) {
		...
		if (!job) {
			...
			if (existsSync(job_file)) {
				var json_raw = readFileSync(job_file, { encoding: 'utf8' });
	...
	rewindJob(job) {
		...
		if (job.catch_up && !job.source && !job.multiplex) {
			var new_start = normalizeTime(job.now - 60, { sec: 0 });
			this.state.cursors[job.event] = new_start;

			var dargs = getDateArgs(new_start);
	...
	finishLocalJob(job) {
		...
		if (job.code > 0 && job.code < 255 && job.retries && !this.server.shut) {
			...
			if (job.retry_delay) blurb += " (" + getTextFromSeconds(job.retry_delay, true, false) + " delay)";
			blurb += "\n\n";

			appendFileSync(job.log_file, blurb);
			...
			if (job.retry_delay) {
				job.when = timeNow() + job.retry_delay;
			}
		...
		try { appendFileSync(job.log_file, footer); }
		catch (err) {
		...
		var stats = null;
		try { stats = statSync(job.log_file); }
		catch (err) {
		...
		if (!self.server.shut) {
			...
			unlink(job.log_file.replace(/\.log$/, '.json'), function (err) { ; });
		}
			...
			writeFileSync(job.log_file.replace(/\.log$/, '.json'), JSON.stringify(job));
		}
	...
	uploadJobLog(job, callback) {
		...
		if (this.multi.manager) {
			...
			stat(job.log_file, function (err, stats) {
				// data will be a stream
				if (err) {
					var data = Buffer.from("(Empty log file)\n");
					writeFileSync(job.log_file, data);
				}

				// get read stream and prepare to compress it
				var stream = createReadStream(job.log_file);
				var gzip = createGzip(self.server.config.get('gzip_opts') || {});
				stream.pipe(gzip);
					...
					if (self.server.config.get('copy_job_logs_to')) {
						...
						self.logger.rotate(job.log_file, dest_path, function (err) {
							if (err) {
								...
								unlink(job.log_file, function (err) { ; });
							}
							else {
	...
	fetchStoreJobLog(job) {
		...
		var api_url = this.getServerBaseAPIURL(worker.hostname, worker.ip) + '/app/fetch_delete_job_log';

		api_url += composeQueryString({
			path: job.log_file,
			auth: digestHex(job.log_file + this.server.config.get('secret_key'))
		});
		...
		job.log_file = this.server.config.get('log_dir') + '/jobs/' + basename(job.log_file);
	...
	finishJob(job) {
		...
		if (!job.time_end) job.time_end = timeNow();
		job.elapsed = Math.max(0, job.time_end - job.time_start);

		var dargs = getDateArgs(job.time_end);
		...
		let expires_at =  timeNow(true) + (86400 * (job.log_expire_days || this.server.config.get('job_data_expire_days')));
		...
		// 	});
		// }
		// log failures via logActivity, rather than above method, ignore silent job
		if (job.code > 0 && job.code != 255 && !job.silent) self.logActivity('job_failure', { job: stub });
		...
		if (!job.silent) { // put non-silent jobs under logs/completed (used for history reports)
			this.storage.enqueue(function (task, callback) {
		...
		if (email_template) {
			...
			email_data.nice_elapsed = getTextFromSeconds(data.elapsed, false, false);
			...
			email_data.nice_log_size = getTextFromBytes(data.log_file_size || 0);
			...
			if (data.mem && data.mem.count) {
				...
				email_data.nice_mem = getTextFromBytes(mem_avg);
				email_data.nice_mem += ' (Peak: ' + getTextFromBytes(data.mem.max) + ')';
			}
			email_data.nice_cpu = '(Unknown)';
			if (data.cpu && data.cpu.count) {
				var cpu_avg = shortFloat(data.cpu.total / data.cpu.count);
				email_data.nice_cpu = '' + cpu_avg + '%';
				email_data.nice_cpu += ' (Peak: ' + shortFloat(data.cpu.max) + '%)';
			}

			// perf may be an object
			if (isaHash(email_data.perf)) email_data.perf = JSON.stringify(email_data.perf);
		...
		// fire web hook
		var hook_data = mergeHashes(data, { action: 'job_complete' });
		...
		if (hook_text_templates[hook_action]) {
			hook_data.text = sub(hook_text_templates[hook_action], hook_data);
		...
		if (job.update_event) {
			this.storage.listFindUpdate('global/schedule', { id: job.event }, job.update_event, function (err) {
				...
				var event_stub = mergeHashes(job.update_event, { id: job.event, title: job.event_title });
	...
	getAllActiveJobs(inc_pending) {
		// gather all active jobs, local and remote
		var jobs = copyHash(this.activeJobs);
		...
		if (inc_pending && this.internalQueue) {
			for (var key in this.internalQueue) {
				...
				if ((task.action == 'launchLocalJob') && task.id && !jobs[task.id]) {
					jobs[key] = mergeHashes(task, { pending: 1 });
				} // is pending job
			} // foreach queue item
		} // internalQueue
		...
		for (var hostname in this.workers) {
			...
			if (worker.active_jobs) {
				mergeHashInto(jobs, worker.active_jobs);
			}
	...
	monitorAllActiveJobs() {
		...
		for (var id in all_jobs) {
			...
			if (job.timeout && (now - job.time_start >= job.timeout)) {
				...
				var nice_timeout = getTextFromSeconds(job.timeout, false, true);
			...
			if (job_memory_max && job.mem) {
				...
				if (current > job_memory_max) {
					...
					if ((now - flagged_jobs[id].mem) >= job_memory_sustain) {
						...
						var msg = "Exceeded memory limit of " + getTextFromBytes(job_memory_max);
						if (job_memory_sustain) msg += " for over " + getTextFromSeconds(job_memory_sustain, false, true);
					...
					if (!numKeys(flagged_jobs[id])) delete flagged_jobs[id];
				}
			} // mem check
			...
			if (job_cpu_max && job.cpu) {
				...
				if (current > job_cpu_max) {
					...
					if ((now - flagged_jobs[id].cpu) >= job_cpu_sustain) {
						...
						if (job_cpu_sustain) msg += " for over " + getTextFromSeconds(job_cpu_sustain, false, true);
			...
			if (job_log_max_size && job.log_file_size && (job.log_file_size > job_log_max_size)) {
				...
				var msg = "Exceeded log file size limit of " + getTextFromBytes(job_log_max_size);
				this.logDebug(4, "Job " + id + " is being aborted: " + msg);
				this.abortJob({ id: id, reason: msg });
				continue;
			}
	...
	monitorServerResources(callback) {
		...
		var finish = function (err, stdout, stderr) {
			...
			for (var id in self.activeJobs) {
				...
				if (pids[job.pid] && (now - job.time_start >= job_startup_grace)) {
					...
					while (numKeys(family) && (++levels <= 100)) {
					...
					if (self.debugLevel(10)) {
						self.logDebug(10, "Active Job: " + job.pid + ": CPU: " + cpu + "%, Mem: " + getTextFromBytes(mem));
					}
		...
		var child = null;
		try {
			child = exec(cmd, { timeout: 5 * 1000 }, finish);
		}
		catch (err) {
			self.logError('job', "Failed to exec ps: " + err);
			if (callback) { callback(); callback = null; }
	...
	watchJobLog(args, socket) {
		...
		series([
			function (callback) {
				// validate auth token
				var correct_token = digestHex(args.id + self.server.config.get('secret_key'));
			...
			function (callback) {
				open(log_file, 'r', function (err, fd) {
					log_fd = fd;
					callback(err);
				});
			},
			function (callback) {
				fstat(log_fd, function (err, stats) {
					log_stats = stats;
					callback(err);
				});
			},
			function (callback) {
				...
				read(log_fd, log_buffer, 0, log_chunk_size, log_pos, function (err, bytesRead, buffer) {
					if (err) return callback(err);
