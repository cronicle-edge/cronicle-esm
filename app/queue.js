...
import { readFile, unlink, writeFile, rename } from "fs";
import { eachSeries } from 'async';
import glob from 'glob';
import { timeNow, generateUniqueID } from "../pixl/tools.mjs";
...
export default class Queue extends Component {
	...
	enqueueExternal(task, callback) {
		// enqueue a task for later (up to 1 minute delay)
		// these may be future-scheduled via 'when' property
		var self = this;
		var task_file = this.server.config.get('queue_dir') + '/' + generateUniqueID(32) + '.json';
		var temp_file = task_file + '.tmp';
		
		this.logDebug(9, "Enqueuing external task", task);
		
		writeFile( temp_file, JSON.stringify(task), function(err) {
			if (err) {
			...
			rename( temp_file, task_file, function(err) {
				if (err) {
