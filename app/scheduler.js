...
import { eachSeries, ensureAsync, whilst } from 'async';
import moment from 'moment-timezone';
import { normalizeTime, timeNow, getDateArgs, getTimeFromArgs, mergeHashes, sub, copyHash } from "../pixl/tools.mjs";
...
export default class Scheduler extends Component {
	...
	setupScheduler() {
		...
		var self = this;
		var now = normalizeTime(timeNow(), { sec: 0 });		
		...
		this.storage.get('global/state', function (err, state) {
			...
			self.storage.listGet('global/schedule', 0, 0, function (err, items) {
				...
				if (queue_event_ids.length) eachSeries(queue_event_ids,
	...
	schedulerMinuteTick(dargs, catch_up_only) {
		...
		if (this.state.enabled) {
			...
			if (!dargs) dargs = getDateArgs(timeNow(true));

			dargs.sec = 0; // normalize seconds
			var now = getTimeFromArgs(dargs);
			...
			self.storage.listGet('global/schedule', 0, 0, function (err, items) {
				...
				eachSeries(items, ensureAsync(function (item, callback) {
					...
					whilst(
						...
						ensureAsync(function (callback) {
							...
							if (self.checkEventTicks(item.ticks, cursor, tz) || timing && self.checkEventTimingMoment(timing, margs)) {
								// item needs to run!
								self.logDebug(4, "Auto-launching scheduled item: " + item.id + " (" + item.title + ") for timestamp: " + margs.format('llll z'));
								self.launchOrQueueJob(mergeHashes(item, { now: cursor }), callback);
							}
							else callback();
						}),
						...
						function (err) {
							if (err) {
								...
								if (!catch_up_only && (timeNow(true) - cursor <= 30) && !err_msg.match(/(Category|Plugin).+\s+is\s+disabled\b/) && !launches[item.id]) {
									self.logActivity('warning', { description: err_msg });
									if (item.notify_fail) {
										self.sendEventErrorEmail(item, { description: err_msg });
									}

									var hook_data = mergeHashes(item, {
	...
	checkEventTiming(timing, cursor, tz) {
	...
	checkEventTicks(tickString, cursor, tz) {
	...
	checkEventTimingMoment(timing, margs) {
	...
	sendEventErrorEmail(event, overrides) {
		...
		var email_data = mergeHashes(event, overrides || {});
	...
	chainReaction(old_job, chain_event_id) {
		...
		this.storage.listFind('global/schedule', { id: chain_event_id }, function (err, event) {
			...
			var job = mergeHashes(copyHash(event, true), {
	...
	checkEventQueues(event_ids, callback) {
	...
	deleteEventQueues(event_ids, callback) {
		// delete one or more event queues
		var self = this;
		if (!Array.isArray(event_ids)) event_ids = [event_ids];

		eachSeries(event_ids,
			function (event_id, callback) {
