...
import { eachSeries, doWhilst } from 'async';
import { join } from 'path';
import * as url from 'url';
const __filename = url.fileURLToPath(import.meta.url);
const __parentdir = url.fileURLToPath(new URL('..', import.meta.url));

import { require, timeNow, escapeRegExp, findObject, getDateArgs, copyHash } from '../pixl/tools.mjs';
...
let setup = require('conf/setup.json')
let version = '2.0.0'
...
setup.storage.forEach( 
	/**
	 * remove uid property on plugins, to avoid using root 
	 * @param {[action: sting, key: string, data: JSON]} item 
	 */
	(item)=> { 
		item[2]
	if(item[2].uid) delete item[2].uid 
})
...
export default class CronicleTest {
	...
	setUp(callback) {
		...
		var pid = false;
		try { pid = readFileSync('logs/unit.pid', { encoding: 'utf8' }); }
		catch (e) { ; }
		if (pid) {
			var alive = true;
		...
		try { execSync('rm -rf logs/unit.pid data/unittest data/unitqueue'); }
		catch (e) { ; }
		...
		server = new PixlServer({
			...
			__version: version,
			config: config,
			components: [ Storage, WebServer, API, User, Engine ]
		...
		server.startup(function () {
			...
			eachSeries(setup.storage, function (params, callback) {
				...
				if (typeof (params[1]) == 'object') {
					var obj = params[1];
					if (obj.created)
						obj.created = timeNow(true);
					if (obj.modified)
						obj.modified = timeNow(true);
					if (obj.regexp && (obj.regexp == '_HOSTNAME_'))
						obj.regexp = '^(' + escapeRegExp(server.hostname) + ')$';
					if (obj.hostname && (obj.hostname == '_HOSTNAME_'))
						obj.hostname = server.hostname;
					if (obj.ip && (obj.ip == '_IP_'))
						obj.ip = server.ip;
				}
	...
	tearDown(callback) {
		...
		setTimeout(function () {
			server.shutdown(function () {
				...
				setTimeout(function () {
					try { execSync('rm -rf data/unittest data/unitqueue'); }
					catch (e) { ; }
	...
	tests = [
		...
		function testAPIGetAPIKeys(test) {
			...
			request.json(api_url + '/app/get_api_keys', params, function (err, resp, data) {
				...
				var api_key = findObject(data.rows, { id: self.apikey_id });
		...
		function testAPIGetSchedule(test) {
			...
			request.json(api_url + '/app/get_schedule', params, function (err, resp, data) {
				...
				var event = findObject(data.rows, { id: self.event_id });
		...
		function testAPIGetJobStatus(test) {
			...
			request.json(api_url + '/app/get_job_status', params, function (err, resp, data) {
				...
				test.ok(job.progress > 0, `Job progress is still non-zero [${job.progress}] `);

				test.ok(!!job.cpu, `Job has CPU metrics [${job.cpu}]`);
				...
				function (callback) {
					...
					request.json(api_url + '/app/get_job_details', params, function (err, resp, data) {
						...
						if (resp.statusCode != 200)
							return callback(new Error("HTTP " + resp.statusCode + " " + resp.statusMessage));

						// e-brake to prevent infinite loop
						if (count++ > 100)
							return callback(new Error("Too many loop iterations polling get_job_details API"));
			...
			doWhilst(
				function (callback) {
					...
					request.json(api_url + '/app/get_event_history', params, function (err, resp, data) {
						...
						// e-brake to prevent infinite loop
						if (count++ > 10)
							return callback(new Error("Too many loop iterations polling get_event_history API"));
			...
			doWhilst(
				function (callback) {
					...
					request.json(api_url + '/app/get_history', params, function (err, resp, data) {
						...
						// e-brake to prevent infinite loop
						if (count++ > 10)
							return callback(new Error("Too many loop iterations polling get_history API"));
			...
			doWhilst(
				function (callback) {
					...
					request.json(api_url + '/app/get_activity', params, function (err, resp, data) {
						...
						// e-brake to prevent infinite loop
						if (count++ > 10)
							return callback(new Error("Too many loop iterations polling get_activity API"));
