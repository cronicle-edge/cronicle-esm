...
import { timeNow, digestHex, mergeHashInto, shortFloat } from "../pixl/tools.mjs";
...
export default class Comm extends Component {
	...
	connectToworker(worker) {
		...
		socket.on('connect', function() {
			...
			var token = digestHex( self.server.hostname + now + self.server.config.get('secret_key') );
		...
		if (!worker.disabled) {
			...
			if (worker.active_jobs) {
				for (var id in worker.active_jobs) {
					self.logDebug(5, "Moving job to limbo: " + id);
					self.deadJobs[id] = worker.active_jobs[id];
					self.deadJobs[id].time_dead = timeNow(true);
				}
				delete worker.active_jobs;
			}
		} // not disabled yet
	...
	checkServerClock(worker) {
		...
		if ((drift >= 10) && (worker.clock_drift < 10)) {
			var err_msg = "Server clock is " + shortFloat(drift) + " seconds out of sync: " + worker.hostname;
	...
	removeServer(server, args) {
		...
		if (worker.active_jobs) {
			for (var id in worker.active_jobs) {
				...
				this.deadJobs[id].time_dead = timeNow(true);
			}
			delete worker.active_jobs;
		}
	...
	handleNewSocket(socket) {
		...
		socket.on('authenticate', function(params) {
			...
			if (params.manager_hostname && params.now && params.token) {
				...
				var correct_token = digestHex( params.manager_hostname + params.now + self.server.config.get('secret_key') );
	...
	restartLocalServer(args) {
		...
		child = spawn( "bin/control.sh", ["restart"], { 
			detached: true,
