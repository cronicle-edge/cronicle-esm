...
import { chmodSync, readFile, existsSync, unlink } from "fs";
import { each, series, eachSeries, whilst} from 'async'
import { scryptSync, randomBytes, createCipheriv, createDecipheriv } from "crypto";
import { execSync } from 'child_process';

import glob from 'glob';
import jstimezonedetect from 'jstimezonedetect';
const determine = jstimezonedetect.determine;
import { parse } from 'dotenv';

import { Expandable } from "../pixl/component.mjs";
import { setPath, mkdirp, timeNow, findObject, numKeys, copyHashRemoveKeys, mergeHashes, isaHash, copyHash, zeroPad } from "../pixl/tools.mjs";
import Request from "../pixl/request.mjs";
import Uncatch from '../pixl/uncatch.mjs'
...
class Engine extends Expandable {
	...
	startup(callback) {
		...
		chmodSync(this.server.config.get('log_dir') + '/jobs', "777");
		chmodSync(this.server.config.get('queue_dir'), "777");
		...
		this.secrets = {};
	    this.eKey = scryptSync(this.server.config.get('secret_key'), '', 32),
		...
		// custom routes/pages (config viewer, job dashboard, etc)
		this.web.addURIHandler(/^\/conf$/i, 'Reports', "./htdocs/custom/config.html");
		...
		// git setup , only for Filesystem storage engine
		// if (self.server.config.get('Storage').Filesystem) {
		// 	this.git = simpleGit(self.server.config.get('Storage').Filesystem.base_dir)
		// 	this.gitlock = 0;
		// }
		...
		Uncatch.on('uncaughtException', this.emergencyShutdown.bind(this));
		...
		this.checkmanagerEligibility(function () {
			...
			self.multi.lastPingReceived = timeNow(true);
		...
		this.storage.listGet('global/conf_keys', 0, 0, function (err, items, list) {
			...
			if (items) { // items only would exist on master 
				for (let i = 0; i < items.length; i++) {
					...
					if (items[i].title) setPath(self.server.config.get(), items[i].title, items[i].key)
				}
	...
	encryptObject(obj) {
		...
		const cipher = createCipheriv(algorithm,  this.eKey, IV);
	...
	decryptObject( encObj ) {
		...
		const decipher = createDecipheriv(algorithm,  this.eKey, IV);
			...
			if (items) { // 
				...
				for (let i = 0; i < items.length; i++) {
					...
					try {
						...
						if(secret.form == 'props' || secret.id == 'globalenv') secret.data = parse(secret.data);
	...
	checkmanagerEligibility(callback) {
		// determine manager server eligibility
		const self = this;
		...
		this.storage.listGet('global/servers', 0, 0, function (err, servers) {
			...
			if (!findObject(servers, { hostname: self.server.hostname }) && !self.multi.cluster) {
	...
	tick() {
		...
		var msr_freq = numKeys(this.activeJobs) ? (this.server.config.get('monitor_res_freq') || 10) : 60;
	...
	updateClientData(name) {
		...
		this.storage.listGet('global/' + name, 0, 0, function (err, items) {
			...
			if (name == 'plugins') { // clear out secrets
				if (Array.isArray(itemCopy)) itemCopy.forEach(plug => delete plug.secret)
			}
	...
	beforeUserLogin(args, callback) {
		...
		each(['schedule', 'categories', 'plugins', 'server_groups'],
	...
	afterUserLogin(args) {
		...
		this.logActivity('user_login', {
			user: copyHashRemoveKeys(args.user, { password: 1, salt: 1 })
		}, args);
	}
	...
	fireInfoHook(web_hook, data, logMessage) {
		...
		if (typeof web_hook === 'string') {
			...
			wh_config = mergeHashes(web_hook, {});
		...
		let wh_options = mergeHashes(self.server.config.get('web_hook_custom_opts') || {}, wh_config.options || {})

		let wh_data = mergeHashes(data || {}, wh_config.data || {}) // 
		...
		if (wh_config.textkey) setPath(wh_data, wh_config.textkey, wh_data.text )
	...
	logActivity(action, orig_data, args) {
		// add event to activity logs async
		const self = this;
		if (!args) args = {};

		assert(isaHash(orig_data), "Must pass a data object to logActivity");
		var data = copyHash(orig_data, true);

		// sanity check: make sure we are still manager
		if (!this.multi.manager) return;

		data.action = action;
		data.epoch = timeNow(true);
	...
	gomanager() {
		...
		this.multi.lastPingSent = timeNow(true);

		// we need to know the server timezone at this point
		this.tz = determine().name();
	...
	managerFailover() {
		...
		this.multi.lastPingReceived = timeNow(true);
				...
				if (file.match(/\/(\w+)(\-detached)?\.json$/)) {
					...
					readFile(file, { encoding: 'utf8' }, function (err, data) {
						...
						if (job) {
							...
							else if (job.detached && existsSync(self.server.config.get('queue_dir') + '/' + job_id + '-complete.json')) {
						...
						eachSeries(files, function (file, callback) {
							...
							function (err) {
								...
								glob(self.server.config.get('log_dir') + '/jobs/*.tmp', {}, function (err, files) {
									...
									eachSeries(files, function (file, callback) { unlink(file, callback); });
								}); // glob
							});
					}); // glob
				}); // eachSeries
	...
	runMaintenance(callback) {
		...
		if (this.state.flagged_jobs && numKeys(this.state.flagged_jobs)) {
	...
	listRoughChop(list_path, max_rows, callback) {
	...
	deleteExpiredJobData(key, data, callback) {
	...
	getUniqueID(prefix) {
		...
		return [
			...
			zeroPad(this._uniqueIDCounter.toString(36), 2)
		].join('');
	}
	...
	emergencyShutdown(err) {
		...
		if(this.server.config.get('admin_web_hook')) {
		  execSync(`curl -X POST -H 'Content-Type: application/json' -d '{"text":"${errorMsg}"}' ${this.server.config.get('admin_web_hook')}`); 
		}
		this.abortAllLocalJobs();
	}
...
Engine.expand({
	__name: 'Cronicle',
	//__parent: Component,
	__mixins: [ API, Comm, Scheduler, Job, Queue, Discovery ],
})
