...
// Server => Engine => API [ Job ]
// Copyright (c) 2015 Joseph Huckaby
// Released under the MIT License

import { readFile, stat, createReadStream, unlink } from 'fs';
import { retry, ensureAsync, series } from 'async';
import { mergeHashes, digestHex, findObjects, timeNow } from "../../pixl/tools.mjs";
...
export default class APIJob extends Component {
	...
	async api_get_live_console(args, callback) {
		// runs on manager 
		const self = this;
		...
		self.loadSession(args, function (err, session, user) {
			...
			let params = mergeHashes(args.params, args.query);
			...
			// activeJobs[params.id] won't work while job is in retry queue, deep scan is needed
			let job = Object.keys(activeJobs).map(key=>activeJobs[key]).filter(e=>e.id == params.id)[0]
			...
			let auth = digestHex(params.id + self.server.config.get('secret_key'))
	...
	api_get_live_log_tail(args, callback) {
		...
		if (params.auth != digestHex(params.id + self.server.config.get('secret_key'))) {
		...
		if (params.download == 1) { // read entire file
			readFile(log_file, { encoding: 'utf-8' }, (err, data) => {
	...
	api_get_live_job_log(args, callback) {
		...
		stat(log_file, function (err, stats) {
			if (err) {
			...
			var stream = createReadStream(log_file);
	...
	api_fetch_delete_job_log(args, callback) {
		...
		if (query.auth != digestHex(query.path + this.server.config.get('secret_key'))) {
			return callback("403 Forbidden", {}, "Authentication failure.\n");
		}
	...
	api_get_log_watch_auth(args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			retry( { times: 20, interval: 250 },
				ensureAsync( function(callback) {
				...
				function(err) {
					...
					// generate token
					var token = digestHex(params.id + self.server.config.get('secret_key'));
	...
	api_update_jobs(args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			var jobs = findObjects(jobs_arr, params);
			var count = 0;
			...
			for (var idx = 0, len = jobs.length; idx < len; idx++) {
				var job = jobs[idx];
				var result = self.updateJob(mergeHashes(updates, { id: job.id }));
				if (result) {
					count++;
	...
	api_get_job_details(args, callback) {
	...
	api_get_job_status(args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			if (job) {
				...
				return callback({
					code: 0,
					job: mergeHashes(job, {
						elapsed: timeNow() - job.time_start
					})
				});
			} // found job
