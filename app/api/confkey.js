...
// Server => Engine => API [ ConfigKeys ]
// Copyright (c) 2015 Joseph Huckaby
// Released under the MIT License

import { timeNow, setPath, deletePath } from "../../pixl/tools.mjs";
import Component from "../../pixl/component.mjs";

export default class APIConfkey extends Component {

	api_get_conf_keys(args, callback) {
	...
	api_get_conf_key(args, callback) {
	...
	api_create_conf_key(args, callback) {
		// add new Config Key
		const self = this;
		let params = args.params;
		if (!this.requiremanager(args, callback)) return;
		...
		this.loadSession(args, async function (err, session, user) {
			...
			self.storage.listUnshift('global/conf_keys', params, function (err) {
				...
				setPath(self.server.config.get(), params.title, params.key); // live update
	...
	api_update_conf_key(args, callback) {
	...
	api_delete_conf_key(args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			self.storage.listFindDelete('global/conf_keys', { id: params.id }, function (err, conf_key) {
				...
				deletePath(self.server.config.get(), params.title); // live update
	...
	// reload all configs
	api_reload_conf_key(args, callback) {
		const self = this;
		let params = args.params;
		if (!this.requiremanager(args, callback)) return;
			...
			self.storage.listGet('global/conf_keys', 0, 0, function (err, items, list) {
				...
				if (items) { // items only would exist on master 
					for (i = 0; i < items.length; i++) {
						if (items[i].title) setPath(config, items[i].title, items[i].key);
						//console.log(items[i]);
					}
				}
