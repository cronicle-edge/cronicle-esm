...
// Server => Engine => API [ Admin ]
// Copyright (c) 2015 Joseph Huckaby
// Released under the MIT License

import { eachSeries, whilst, queue as _queue } from 'async';
import { digestHex, timeNow, normalizeTime, mergeHashInto } from "../../pixl/tools.mjs";
import { Readable } from "stream";
import Component from '../../pixl/component.mjs';
//import { CronicleServer, Multi } from '../../@types/global.js';


// Servers / manager Control
export default class APIAdmin extends Component {

	api_check_add_server(args, callback) {
		...
		if (params.token != digestHex( params.manager + params.now + this.server.config.get('secret_key') )) {
	...
	api_add_server(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var api_args = {
				...
				now: now,
				token: digestHex( self.server.hostname + now + self.server.config.get('secret_key') )
			};
	...
	api_remove_server(args, callback) {
	...
	api_restart_server(args, callback) {
	...
	api_shutdown_server(args, callback) {
	...
	api_update_manager_state(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (params.enabled) {
				...
				self.storage.listGet( 'global/schedule', 0, 0, function(err, items) {
					...
					mergeHashInto( self.state, params );
	...
	api_import(args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			var queue = _queue(function (line, callback) {
				// process each line
