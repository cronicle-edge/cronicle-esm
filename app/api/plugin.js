...
// Server => Engine => API [ Plugin ]
// Copyright (c) 2015 Joseph Huckaby
// Released under the MIT License

import { parse as sqparse } from 'shell-quote';
import { timeNow, getDateArgs } from "../../pixl/tools.mjs";
...
export default class APIPlugin extends Component {
	...
	api_create_plugin(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			plugin.created = plugin.modified = timeNow(true);
	...
	api_update_plugin(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.storage.listFind( 'global/plugins', { id: params.id }, function(err, plugin) {
				...
				// avoid erasing secret value on save, if value is not provided
				params.secret = params.secret || plugin.secret;
	...
	requireValidPluginCommand(command, callback) {
