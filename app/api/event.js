...
// Server => Engine => API [ Event ]
// Copyright (c) 2015 Joseph Huckaby
// Released under the MIT License

import { mergeHashes, timeNow, normalizeTime, getDateArgs, copyHash, mergeHashInto } from "../../pixl/tools.mjs";
import { createHmac } from 'crypto';
import Component from "../../pixl/component.mjs";

export default class APIEvent extends Component {

	api_get_schedule(args, callback) {
	...
	api_create_event(args, callback) {
		...
		this.loadSession(args, async function (err, session, user) {
			...
			event.created = event.modified = timeNow(true);
			...
			self.storage.listFind('global/schedule', { id: params.id }, async function (err, event) {
				...
				self.storage.listFind('global/categories', { id: params.category || event.category }, function (err, item) {
					...
					if (params.reset_cursor) {
						new_cursor = normalizeTime(params.reset_cursor - 60, { sec: 0 });
					...
					self.storage.listFindUpdate('global/schedule', { id: params.id }, params, function (err) {
						...
						if (new_cursor) {
							var dargs = getDateArgs(new_cursor);
		...
		this.loadSession(args, function (err, session, user) {
			...
			self.storage.listFind('global/schedule', { id: params.id }, function (err, event) {
				...
				self.storage.listFindDelete('global/schedule', { id: params.id }, function (err) {
					...
					self.storage.expire('logs/events/' + event.id, timeNow(true) + 86400);
	...
	api_run_event(args, callback) {
		...
		var params = copyHash(args.query, true);
		if (args.query.post_data) params.post_data = args.params;
		else mergeHashInto(params, args.params);
		...
		this.loadSession(args, function (err, session, user) {
			...
			self.storage.listFind('global/schedule', criteria, function (err, event) {
				...
				if (user.token) { // token auth
					...
					let expectedToken = createHmac("sha1", `${self.server.config.get("secret_key")}`)
				...
				var job = mergeHashes(copyHash(event, true), params);
				if (user.key) {
					// API Key
