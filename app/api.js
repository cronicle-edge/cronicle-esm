...
import { timeNow, isaArray, copyHash } from "../pixl/tools.mjs";
...
import APIJob from "./api/job.js";
import APIAdmin from "./api/admin.js";
import APIApikey from "./api/apikey.js";
import APIConfkey from "./api/confkey.js";
import APISecret from "./api/secret.js";

class API extends Expandable {
	...
	getServerBaseAPIURL(hostname, ip) {
	...
	requireValidEventData(event, callback) {
	...
	requireGroupPrivilege(args, user, grp_id, callback) {
	...
	getClientInfo(args, params) {
		...
		var info = null;
		if (params) info = copyHash(params, true);
		else info = {};
	...
	async validateUnique(listName, obj, keyArray) {
	...
	loadSession(args, callback) {
		...
		if (wfsig && wfid) {
			...
			if(wfJob && wfsig == `sha1=${createHmac("sha1", self.server.config.get('secret_key')).update(wfid).digest("hex")}`) {
				return callback(null
				, { type: "workflow", workflow: true, username: "internal" }
				, { username: "internal", workflow: wfJob.event, signature: wfsig, active: true, privileges: { run_events: 1, abort_events: 1 } }
				);
			}
			else {
				return callback(new Error(`Invalid/Expired WorkFlow Signature`), null, null);
			}
		}
		...
		if (args.request.headers['x-hub-signature']) {
			...
			let expectedSignature = "sha1=" + createHmac("sha1", `${key}`)
