...
import { timeNow, generateUniqueID, copyHash, copyHashRemoveKeys, normalizeTime, mergeHashes, digestHex } from "./tools.mjs";
import Mailer from './mail.mjs';
import Request from './request.mjs';
import { hashSync, compareSync } from 'bcrypt-node';
...
export default class User extends Component {
	...
	api_create = function(args, callback) {
		...
		this.storage.get(path, function(err, old_user) {
			...
			user.active = 1;
			user.created = user.modified = timeNow(true);
			user.salt = generateUniqueID( 64, user.username );
			user.password = self.generatePasswordHash( user.password, user.salt );
			user.privileges = copyHash( self.config.get('default_privileges') || {} );
				...
				self.storage.put( path, user, function(err) {
					if (err) {
						...
						self.logTransaction('user_create', user.username, 
							self.getClientInfo(args, { user: copyHashRemoveKeys( user, { password: 1, salt: 1 } ) }));
	...
	api_login = async function (args, callback) {
		...
		if (user.ext_auth) { // do AD auth
			...
			// override default domain if username contains (e.g. user@domain.com)
		...
		if (!isValidPassword) {
			...
			var date_code = Math.floor(timeNow() / 3600);
			...
			self.storage.put('users/' + self.normalizeUsername(params.username), user, function () {
				return self.doError('login', "Username or password incorrect.", callback); // deliberately vague
			});
		...
		self.fireHook('before_login', args, function (err) {
			...
			var expiration_date = normalizeTime(
				now + (86400 * self.config.get('session_expire_days')),
				{ hour: 0, min: 0, sec: 0 }
			);

			// create session id and object
			var session_id = generateUniqueID(64, params.username);
			var session = {
				id: session_id,
			...
			self.storage.put('sessions/' + session_id, session, function (err) {
				if (err) {
					...
					callback(mergeHashes({
						code: 0,
						username: user.username,
						user: copyHashRemoveKeys(user, { password: 1, salt: 1 }),
	...
	api_logout = function (args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			self.fireHook('before_logout', args, function (err) {
				...
				self.storage.delete('sessions/' + session.id, function () {
		...
		this.loadSession(args, function (err, session, user) {
			...
			self.fireHook('before_update', args, function (err) {
				...
				if (updates.new_password) {
					updates.salt = generateUniqueID(64, user.username);
				...
				// update user record
				user.modified = timeNow(true);

				self.logDebug(6, "Updating user", user);

				self.storage.put("users/" + self.normalizeUsername(user.username), user, function (err) {
					if (err) {
						return self.doError('user', "Failed to update user: " + err, callback);
					}
		...
		this.loadSession(args, function (err, session, user) {
			...
			self.fireHook('before_delete', args, function (err) {
				...
				self.storage.delete('sessions/' + session.id, function () {
					...
					self.storage.delete("users/" + self.normalizeUsername(user.username), function (err) {
						if (err) {
	...
	api_forgot_password = function (args, callback) {
		...
		this.storage.get('users/' + this.normalizeUsername(params.username), function (err, user) {
			...
			self.fireHook('before_forgot_password', args, function (err) {
				...
				var recovery_key = generateUniqueID(64, user.username);

				// dates
				var now = timeNow(true);
				var expiration_date = normalizeTime(now + 86400, { hour: 0, min: 0, sec: 0 });
				...
				self.storage.put('password_recovery/' + recovery_key, recovery, function (err) {
					if (err) {
			...
			self.storage.get('password_recovery/' + params.key, function (err, recovery) {
				...
				self.fireHook('before_reset_password', args, function (err) {
					...
					self.storage.put("users/" + self.normalizeUsername(user.username), user, function (err) {
						...
						self.storage.delete('password_recovery/' + params.key, function () {
	...
	api_admin_create = function (args, callback) {
		...
		this.loadSession(args, function (err, session, admin_user) {
			...
			self.storage.get(path, function (err, old_user) {
				...
				// now we can create the user
				new_user.active = 1;
				new_user.created = new_user.modified = timeNow(true);
				new_user.salt = generateUniqueID(64, new_user.username);
				new_user.password = self.generatePasswordHash(new_user.password, new_user.salt);
				new_user.privileges = new_user.privileges || copyHash(self.config.get('default_privileges') || {});
				...
				self.fireHook('before_create', args, function (err) {
					...
					self.storage.put(path, new_user, function (err) {
						if (err) {
							return self.doError('user', "Failed to create user: " + err, callback);
						}
						else {
							...
							self.logTransaction('user_create', new_user.username,
								self.getClientInfo(args, { user: copyHashRemoveKeys(new_user, { password: 1, salt: 1 }) }));
	...
	api_admin_get_user = function (args, callback) {
		...
		var self = this;
		var params = mergeHashes(args.params, args.query);
			...
			self.storage.listGet('global/users', params.offset, params.limit, function (err, stubs, list) {
				...
				self.storage.getMulti(paths, function (err, users) {
					...
					for (var idx = 0, len = users.length; idx < len; idx++) {
						users[idx] = copyHashRemoveKeys(users[idx], { password: 1, salt: 1 });
					}
					...
					self.storage.get(path, function (err, user) {
						...
						if (!user) {
							...
							user = {
								username: username,
								active: 1,
								created: timeNow(true),
								modified: timeNow(true),
								salt: generateUniqueID(64, username),
								password: generateUniqueID(64), // unused
								privileges: copyHash(self.config.get('default_privileges') || {})
							};
						} // new user
						else {
						...
						self.storage.put(path, user, function (err) {
							...
							self.fireHook('before_login', args, function (err) {
								...
								// create session id and object
								var session_id = generateUniqueID(64, username);
								var session = {
									id: session_id,
									username: username,
									ip: args.ip,
	...
	generatePasswordHash = function (password, salt) {
		...
		if (this.config.get('use_bcrypt')) {
			...
			return digestHex(password + salt, 'sha256');
		}
	}
	...
	comparePasswords = function (password, hash, salt) {
		...
		if (this.config.get('use_bcrypt')) {
			// use extremely secure but CPU expensive bcrypt algorithm
			return compareSync(password + salt, hash);
		}
		else {
			// use weaker but fast salted SHA-256 algorithm
