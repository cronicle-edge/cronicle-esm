...
//import { create } from "./class.mjs";
import Component from "./component.mjs";
import { escapeRegExp, isaHash, parseQueryString } from "./tools.mjs";
...
export default class API extends Component {
	...
	addNamespace(name, prefix, obj) {
		// add namespace (API class), e.g. /api/user/add
		name = this.normalizeAPIName(name);
		this.logDebug(3, "Adding API namespace: " + name);
		this.namespaces[name] = {
			regexp: new RegExp( escapeRegExp(this.config.get('base_uri') + "/" + name + "/" ) + "(\\w+)" ),
			prefix: prefix || "",
			obj: obj
		};
	}
	...
	startup (callback) {
		...
		var regex = new RegExp( escapeRegExp(this.config.get('base_uri')) + "/(\\w+)" );
	...
	handler(args, callback) {
		...
		if (!isaHash(args.params)) args.params = {};
