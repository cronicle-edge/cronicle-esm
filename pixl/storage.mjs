...
import { queue, eachLimit, whilst } from 'async';
import unidecode from 'unidecode';
import { require, getDateArgs, normalizeTime, timeNow, numKeys    } from "./tools.mjs";
import Perf from "./perf.mjs";

import StorageCore from "./storage-core.mjs";
import FSEngine from './engines/Filesystem.js';
import S3Engine from './engines/S3.js'
import RedisEngine from './engines/Redis.js';
// import RedisClusterEngine from './engines/RedisCluster.js';
// import CouchbaseEngine from './engines/Couchbase.js';
...
export default class Storage extends StorageCore {
	...
	startup = function(callback) {
		...
		let engine = this.config.get('engine')

		let engines = {
			Filesystem: FSEngine,
			S3: S3Engine,
			Redis: RedisEngine,
			// RedisCluster: RedisClusterEngine
		}

		let StorageEngine = engines[engine] || require(this.config.get('engine_path'))
		...
		this.queue = queue( this.dequeue.bind(this), this.concurrency );
	...
	putMulti = function(records, callback) {
		...
		eachLimit(Object.keys(records), this.concurrency, 
	...
	headMulti = function(keys, callback) {
		...
		eachLimit(keys, this.concurrency, 
			function(key, callback) {
				// iterator for each key
	...
	rename = function(old_key, new_key, callback) {
		...
		this.copy( old_key, new_key, function() {
	...
	expire = function(key, expiration, force) {
		...
		var dargs = getDateArgs(
			normalizeTime( expiration, { hour:0, min:0, sec:0 } ) 
		);
		
		var dnow = getDateArgs( new Date() );
		if (!force && ((dargs.epoch <= dnow.epoch) || (dargs.yyyy_mm_dd == dnow.yyyy_mm_dd))) {
			...
			dargs = getDateArgs( normalizeTime(
				normalizeTime( dnow.epoch, { hour:12, min:0, sec:0 } ) + 86400,
				{ hour:0, min:0, sec:0 } )
			);
		}
	...
	dequeue = function(task, callback) {
		...
		switch (task.action) {
			case 'expire_set':
				...
				var dargs = getDateArgs( task.expiration );
				var cleanup_list_path = '_cleanup/' + dargs.yyyy + '/' + dargs.mm + '/' + dargs.dd;
				var cleanup_hash_path = '_cleanup/expires';
				
				this.listPush( cleanup_list_path, { key: task.key }, { page_size: 1000 }, function(err) {
	...
	runMaintenance = function(date, callback) {
		...
		var self = this;
		var dargs = getDateArgs( date || (new Date()) );
		var cleanup_list_path = '_cleanup/' + dargs.yyyy + '/' + dargs.mm + '/' + dargs.dd;
		var cleanup_hash_path = '_cleanup/expires';
		var stats = {
			time_start: timeNow(),
		...
		var deleteExpiredRecord = function(key, callback) {
			...
			var finishDelete = function(err) {
				...
				if (self.expHash) {
					self.hashDelete( cleanup_hash_path, key, function() { 
				...
				if (self.expHash) {
					self.hashGet( cleanup_hash_path, key, function(err, data) {
						if (data && data.expires) {
							var eargs = getDateArgs( data.expires );
				...
				if (err) {
					...
					self.listDelete( cleanup_list_path, true, function(err) {
						...
						self.engine.runMaintenance( function() {
							stats.elapsed_sec = timeNow() - stats.time_start;
	...
	waitForAllLocks = function(callback) {
		...
		var self = this;
		var num_locks = numKeys(this.locks);
	...
	tick = function() {
		...
		if (numKeys(metrics) && (this.logEventTypes.perf_sec || this.logEventTypes.all)) {
			this.logger.print({ 
	...
	tickMinute = function() {
		...
		if (numKeys(metrics) && (this.logEventTypes.perf_min || this.logEventTypes.all)) {
			this.logger.print({ 
				component: this.__name,
				category: 'perf', 
				code: 'minute', 
