...
import { openSync, appendFileSync, readFileSync, writeFileSync, statSync, stat, unlinkSync } from 'fs';
import { hostname as _hostname } from 'os';
import { eachSeries } from 'async';

import Logger from "./logger.mjs";
import Config from "./config.mjs";
import { mkdirp, alwaysArray, timeNow, getDateArgs } from "./tools.mjs";
import Args from "./args.mjs";

import daemon from 'daemon'
import Uncatch from './uncatch.mjs';
import { EventEmitter } from 'events';

export default class PixlServer extends EventEmitter {
	...
	__init(callback) {
		...
		// --config param will be set using _main.mjs or cronicle.mjs
		...
		this.logger = new Logger(
			join( (this.config.get('log_dir') || '.'), (this.config.get('log_filename') || 'event.log') ),
			this.config.get('log_columns') || ['hires_epoch', 'date', 'hostname', 'pid', 'component', 'category', 'code', 'msg', 'data'],
			{ echo: this.echo, color: this.color, hostname: _hostname() }
		);
		...
		if (!this.debug && !this.foreground) {
			...
			if (this.config.get('stdout')) {
				daemon_opts.stdout = openSync(this.config.get('stdout'), 'a');
			}
			if (this.config.get('stderr')) {
				daemon_opts.stderr = openSync(this.config.get('stderr'), 'a');
			}
			daemon(daemon_opts);
		...
		if (!this.debug) {
			...
			if (this.config.get('log_crashes')) {
				Uncatch.on('uncaughtException', function(err) {
					appendFileSync( join(self.config.get('log_dir'), self.config.get('crash_filename') || 'crash.log'),
						(new Date()).toString() + " - " + _hostname() + " - PID " + process.pid + "\n" + 
						err.stack + "\n\n"
					);
		...
		if (this.config.get('pid_file')) {
			...
			var pid = 0;
			try { pid = parseInt( readFileSync( pid_file, 'utf8' ) ); } catch (e) {;}
			...
			try {
				pid = readFileSync( pid_file, 'utf8' );
	...
	setupMultiConfig() {
		...
		if (self.args.multiConfig) {
			var files = alwaysArray( self.args.multiConfig );
	...
	applyConfigOverrides() {
		...
		if (this.config.get('config_overrides_file')) {
			...
			this.coModTime = 0;
			this.coCheckTime = timeNow(true);
			this.configOverrides = null;
			try {
				var stats = statSync( this.coFile );
			...
			if (this.coModTime) {
				...
				try {
					this.configOverrides = JSON.parse( readFileSync( this.coFile, 'utf8' ) );
				}
				catch (err) {
	...
	tick() {
		...
		if (this.coModTime && (dargs.epoch - this.coCheckTime >= this.config.freq / 1000)) {
			...
			stat( this.coFile, function(err, stats) {
	...
	shutdown(callback) {
		...
		if (this.config.get('pid_file')) {
			try {
				var pid = readFileSync( this.config.get('pid_file'), 'utf8' );
				this.logDebug(9, "Deleting PID File: " + this.config.get('pid_file') + ": " + pid);
				unlinkSync( this.config.get('pid_file') );
			}
			catch (e) {;}
		}
		...
		// stop components
		eachSeries(this.components.reverse(), 
			function(comp, callback) {
				// stop component
