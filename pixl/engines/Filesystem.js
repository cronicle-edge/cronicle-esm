...
import { mkdirp, copyHashRemoveKeys, digestHex, substitute, timeNow } from "../../pixl/tools.mjs";
import Cache from "../../pixl/cache.mjs";

import { dirname, basename } from 'path';
import { rename, writeFile, createWriteStream, stat, readFile, createReadStream, unlink, rmdir, open, fsync, close, readdir } from 'fs';
import { whilst, eachSeries } from 'async';


export default class FSEngine extends Component {
	...
	setup = function() {
		...
		if (cache_opts && cache_opts.enabled) {
			...
			this.cache = new Cache( copyHashRemoveKeys(cache_opts, { enabled: 1 }) );
	...
	getFilePath = function(key) {
		...
		if (this.rawFilePaths) {
			...
			if (this.keyTemplate) {
				...
				file = dir + '/' + substitute( temp, { key: key, md5: md5 } );
			}
			else {
	...
	_renameFile = function(source_file, dest_file, callback) {
		...
		rename(source_file, dest_file, function(rn_err) {
			...
			self._makeDirs( dirname(dest_file), 0o0775, function(mk_err) {
				if (mk_err) return callback(rn_err);
				
				// last try
				rename(source_file, dest_file, callback);
			});
		});
	}
	...
	put = function(key, value, callback) {
		...
		var temp_file = this.tempDir + '/' + basename(file) + '.tmp.' + this.tempFileCounter;
		this.tempFileCounter = (this.tempFileCounter + 1) % 10000000;
		
		// write temp file (atomic mode)
		writeFile( temp_file, value, function (err) {
			if (err) {
			...
			self._makeDirs( dir, 0o0775, function(err) {
				...
				self._renameFile( temp_file, file, function (err) {
					...
					if (self.cache && !is_binary) {
						self.cache.set( key, value, { date: timeNow(true) } );
					}
	...
	putStream = function(key, inp, callback) {
		...
		var outp = createWriteStream( temp_file );
	...
	putStreamCustom = function(key, inp, opts, callback) {
		...
		// create the write stream to temp file
		var outp = createWriteStream( temp_file, opts || {} );
	...
	get = function(key, callback) {
		...
		readFile(file, opts, function (err, data) {
			if (err) {
				if (err.message.match(/ENOENT/)) {
					err.message = "File not found";
					err.code = "NoSuchKey";
			...
			// possibly cache in LRU
			if (self.cache && !is_binary) {
				self.cache.set( key, data, { date: timeNow(true) } );
			}
	...
	getStreamRange = function(key, start, end, callback) {
		...
		stat(file, function(err, stats) {
			...
			// create read stream
			var inp = createReadStream( file, { start, end } );
	...
	runMaintenance = function(callback) {
		...
		readdir( this.tempDir, function(err, files) {
