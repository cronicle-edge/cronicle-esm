...
import { copyHashRemoveKeys, digestHex, sub, timeNow } from "../../pixl/tools.mjs";
...
export default class S3Engine extends Component {
	...
	prepKey = function (key) {
		...
		if (this.keyTemplate) {
			...
			key = sub(temp, { key: key, md5: md5, ns: ns });
		}
	...
	get = async function (key, callback) {
		...
		try {
			...
			if (is_binary) {
				...
				if (self.cache) {
					self.cache.set(orig_key, body, { date: timeNow(true) });
				}
