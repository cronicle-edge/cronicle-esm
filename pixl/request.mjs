...
import https, { Agent as _Agent } from 'https';
import { stringify } from 'querystring';
import {BrotliCompress, createBrotliDecompress, createGunzip, createInflate, brotliDecompress, gunzip, inflate } from 'zlib';
import _url from 'url';
import FormData from 'form-data';
import { parse, compose_xml as _stringify } from './xml.mjs';
import Perf from './perf.mjs';
import { code } from 'errno';
import { _checkIsHttpToken, _checkInvalidHeaderChar } from '_http_common';
import { EventEmitter } from 'events';

// sniff for Brotli compression support, as it was added in Node v10.16
let hasBrotli = !!BrotliCompress;

let pixlreq_agent = "PixlRequest " + "1.0.0";
let dns_cache = {};

let checkIsHttpToken = _checkIsHttpToken;
let checkInvalidHeaderChar = _checkInvalidHeaderChar;
	...
	post = function(url, options, callback) {
		...
		if ((typeof(options.data) == 'object') && !is_buffer) {
			...
			if (options.json) {
				...
				// XML REST
				options.data = _stringify(options.data, options.xmlRootNode || 'Request') + "\n";
				...
				if (options.files) {
					for (key in options.files) {
						...
						if (typeof(file) == 'string') {
							...
							form.append( key, createReadStream(file) );
						}
						else if (Array.isArray(file)) {
							// array of [file path or stream or buffer, filename]
							var file_data = file[0];
							if (typeof(file_data) == 'string') file_data = createReadStream(file_data);
				...
				// form urlencoded
				options.data = Buffer.from( stringify(options.data) );
	...
	request = function(url, options, callback) {
		...
		if ('download' in options) {
			...
			if (typeof(download) == 'string') {
				try { download = createWriteStream(download); }
				catch (err) {
				...
				res.on('end', function() {
					...
					if (total_bytes) {
						...
						if (self.autoDecompress && res.headers['content-encoding'] && res.headers['content-encoding'].match(/\bbr\b/i) && hasBrotli && callback) {
							// brotli decompress
							brotliDecompress( buf, function(zerr, data) {
							...
							// inflate data first
							inflate( buf, function(zerr, data) {
								perf.end('decompress', perf.perf.total.start);
								if (!callback_fired) {
									callback_fired = true;
									callback( err || zerr, res, data, self.finishPerf(perf) );
		...
		req.on('error', function(e) {
			...
			if (callback && !aborted) {
				...
				else if (e.errno && code[e.errno]) {
					msg = ucfirst(code[e.errno].description) + " (" + e.message + ")";
				}
				if (timer) { clearTimeout(timer); timer = null; }
				if (!callback_fired) {
					// check for retry
