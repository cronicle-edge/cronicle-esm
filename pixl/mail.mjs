...
import { sub, numKeys } from './tools.mjs';
// import { EventEmitter } from 'events';

export default class PixlMail { // extends EventEmitter
	...
	send = function(data, args, callback) {
		...
		if (!data.match(/\n/)) {
			readFile(data, { encoding: 'utf8' }, function (err, data) {
				if (err) callback(err);
		...
		// setup transport
		var transport = createTransport(this.options);
		...
		// custom headers
		if (numKeys(headers)) opts.headers = headers;
